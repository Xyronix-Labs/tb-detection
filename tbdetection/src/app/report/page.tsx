...
type Symptoms = Record<string, boolean>;
type MedicalHistory = Record<string, string | number | boolean>;
...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    if (u && s && m) {
      setUserInfo(JSON.parse(u) as UserInfo);
      setSymptoms(JSON.parse(s) as Symptoms);
      setMedicalHistory(JSON.parse(m) as MedicalHistory);
    } else {
      ...
      <div className="bg-gray-900 text-white shadow-2xl rounded-2xl p-6 sm:p-10 w-full max-w-5xl">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-800 p-4 rounded-lg">
            ...
            <p>
              <span className="font-medium">Weight:</span> {userInfo.weight} kg
            </p>
            <p>
              <span className="font-medium">Height:</span> {userInfo.height} cm
            </p>
            <p>
          ...
          <button
            onClick={async () => {
              const newReport = {
                ...
                notes: `Symptoms: ${JSON.stringify(
                  symptoms
                )}, History: ${JSON.stringify(medicalHistory)}`,
              };
